...
import { archiveDashboardCategory, getDashboardCategory, updateDashboardCategory } from "../../../../../modules/products/admin-catalog.service";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    return Response.json(ok(await getDashboardCategory((await context.params).id)));
  } catch (error) {
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return Response.json(ok(await updateDashboardCategory((await context.params).id, await request.json(), actorUserId)));
  } catch (error) {
    return toError(error);
  }
}
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const actorUserId = access.actorUserId;
    await archiveDashboardCategory((await context.params).id, actorUserId);
    return Response.json(ok({ archived: true }));
  } catch (error) {
    return toError(error);
  }
}