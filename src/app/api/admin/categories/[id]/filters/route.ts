...
import { createDashboardCategoryFilter, listDashboardCategoryFilters } from "@/modules/products/admin-catalog.service";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    return Response.json(ok(await listDashboardCategoryFilters((await context.params).id)));
  } catch (error) {
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return Response.json(ok(await createDashboardCategoryFilter((await context.params).id, await request.json(), actorUserId)), { status: 201 });
  } catch (error) {
    return toError(error);
  }
}