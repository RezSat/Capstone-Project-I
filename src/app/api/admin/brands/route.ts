...
import { createDashboardBrand, listDashboardBrands } from "../../../../modules/products/admin-catalog.service";
...
export async function GET() {
  ...
  try {
    return Response.json(ok(await listDashboardBrands()));
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  if (access.denied) return access.denied;
  try {
    return Response.json(ok(await createDashboardBrand(await request.json())), { status: 201 });
  } catch (error) {
    return toError(error);
  }
}
