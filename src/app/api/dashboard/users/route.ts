...
import { createDashboardUserService, listDashboardUsersService } from "../../../../modules/users/dashboard-users.service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await createDashboardUserService(input);
...
export async function GET() {
  ...
  if (result.status === "error") {
    return Response.json(fail("INTERNAL_ERROR", "Failed to list staff users"), { status: 500 });
  }
  return Response.json(ok(result.items));
}
